...
public class CommodityResource {
	...
	public Response login(@QueryParam("nzd") String userid, @QueryParam("page") @DefaultValue("0") int page,
	...
	@Path("/{cid}")
	@Produces("application/json;charset=UTF-8")
	public Response login(@PathParam("cid") int cid) {
		try {
			Commodity result = commodityService.getCommodity(cid);
			return Response.status(Status.OK).entity(result).type(MediaType.APPLICATION_JSON).build();
		} catch (Exception e) {
			return Response.status(Status.INTERNAL_SERVER_ERROR).entity(e.getMessage()).build();
		}
