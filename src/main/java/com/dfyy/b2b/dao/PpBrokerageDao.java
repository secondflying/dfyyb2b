...
public interface PpBrokerageDao extends CrudRepository<PpBrokerage, Integer>, JpaSpecificationExecutor<Integer> {
	...
	@Query(value="SELECT t.*,p.platform,p.partner,p.id as pid FROM b2b_commodity_type t left outer join b2b_pp_brokerage p on (t.id=p.cid and p.status=0) where t.status=0 order by t.id asc",nativeQuery=true)//原生sql语句
