...
public interface CommodityDao extends CrudRepository<Commodity, Integer>, JpaSpecificationExecutor<Commodity> {
	...
	@Query("select u from Commodity as u left join fetch u.provider left join fetch u.type left join fetch u.unit where u.provider.id=?1 order by u.time desc ")
	public List<Commodity> getByUser(String userid);
}
