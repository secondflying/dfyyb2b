...
public interface OrdersDao extends CrudRepository<Orders, Integer>, JpaSpecificationExecutor<Orders> {
	...
	@Query("select u from Orders as u left join fetch u.nzd  left join fetch u.commodity  where u.commodity.provider.id in (?1) and u.time > ?2 and u.status = 0 order by u.time desc ")
	public List<Orders> getNewByProvider(String[] userids, Date time);
