...
<c:set var="pageTitle" value="编辑田间地头" scope="request" />
<jsp:include page="../includes/header.jsp" />

<section class="well shadow">
	<c:url var="addUrl" value="editone?page=${page}&size=${size}" />
	<form id="agrotechnical2" action="${addUrl}" method="post">
		<fieldset>
			<legend>田间地头编辑
			...
			<div class="control-group">
				...
				<input type="hidden" path="id" name='id' id="id" class="input-xxlarge" value="${agrotechnical.id}" />
				<div class="controls">
					<input path="title" name='title' id="title" class="input-xxlarge" value="${agrotechnical.title}" placeholder="输入标题"  style="width:100%;" />
				...
				<div class="controls">
					<select class="selectpicker" name="category.id" id="category.id" path="category.id"  style="width:100%;">
						<c:forEach items="${cates}" var="cate">
							<option value="${cate.id}" <c:if test="${not empty agrotechnical.category and agrotechnical.category.id == cate.id}">selected</c:if>><c:out
			...
			<div class="control-group">
				...
				<div class="controls">
					<textarea rows="20" path="content" name='content' id="content" style="width:100%;" >${agrotechnical.content1}</textarea>
					<span class="help-inline"></span>
				</div>
			</div>
		</fieldset>
