...
<c:set var="pageTitle" value="配方订单" scope="request" />
				...
				<c:forEach items="${orders}" var="order">
					<tr class="${cssClass}">
						<td style="width: 120px;"><img style="width: 120px; height: 90px;"
							src="${imageUrl}/${order.recipe.thumbnail}" id="recipeImage" /></td>
						<td><c:out value="${order.recipe.title}" /> <br /> ￥<c:out value="${order.recipe.newprice}" /></td>
						...
						<td><c:choose>
								...
								<c:when test="${order.status==1}">
									<span class="label label-default" style="width: 60px; text-align: center; background-color: #5cb85c">待评价</span>
								</c:when>
...
$(document).ready(function () {
	...
	var options = {
		...
		onPageClicked : function (event, originalEvent, type, page) {
			var page = page - 1;
			window.location.href = '<c:url value="/nzd/orders" />?page=' + page + "&size=" + size;
		}
	};
