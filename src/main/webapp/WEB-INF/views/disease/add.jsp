...
<c:set var="pageTitle" value="新增病虫害信息" scope="request" />
<jsp:include page="../includes/header.jsp" />

<section class="well shadow">
	<c:url var="addUrl" value="addone" />
	<form id="disease" action="${addUrl}" method="post">
		<fieldset>
			<legend>
				病虫害信息
			...
			<div class="control-group">
				...
				<div class="controls">
					<input path="name" name='name' id="name" class="input-xxlarge" placeholder="输入病虫害名称" />
			...
			<div class="control-group">
				...
				<div class="controls">
					...
					<c:forEach items="${crops}" var="crop">
						<option value="${crop.id}"<c:if test="${not empty disease.crop and crop.id == disease.crop.id}">selected</c:if>><c:out value="${crop.name}" /></option>
					</c:forEach>
					</select>
					<span class="help-inline"></span>
				</div>
