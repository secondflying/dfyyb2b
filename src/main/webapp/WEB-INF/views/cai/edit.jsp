...
<c:set var="pageTitle" value="编辑猜农资" scope="request" />
	...
	<form id="second" action="${addUrl}" method="post">
		<fieldset>
			<legend>
				...
				<input type="hidden" path="id" name='id' id="id" class="input-xxlarge" value="${cai.id}" />
			</legend>
			<div class="divnull">&nbsp;&nbsp;</div>
			<div class="control-group">
				<label class="control-label" for="name">内容</label>
				<div class="controls">
					<textarea rows="5" path="content" name='content' id="content" class="span8" placeholder="输入竞猜内容">${cai.content }</textarea>
			...
			<div class="control-group">
				...
				<div class="controls">
					...
					<div id="attachmentsdiv1">
						<c:set value="0" var="sum" />
						<c:forEach items="${cai.attachments1}" var="attachment">
							<div id="panel${sum}"
								...
								<input type="hidden" path="attachments1[${sum}].id" name="attachments1[${sum}].id" id="attachments1[${sum}].id"
									value="${attachment.id}" />
								<input type="hidden" path="attachments1[${sum}].url" name="attachments1[${sum}].url"
									id="attachments1[${sum}].url" value="${attachment.url}" />
							</div>
			...
			<div class="control-group">
				...
				<div class="controls">
					<input path="answer" name='answer' id="answer" class="input-xxlarge" value="${cai.answer}" placeholder="输入竞猜答案" />
					<span class="help-inline"></span>
				</div>
			</div>

			<div class="control-group">
				<label class="control-label" for="content">答案图片22</label>
				<div class="controls">
					<input type="file" name="uploadify" id="multiple_file_upload2" />
					<div id="attachmentsdiv2">
						<c:set value="0" var="sum" />
						<c:forEach items="${cai.attachments2}" var="attachment">
							<div id="panel2${sum}"
								style="width: 150px; height: 150px; display: inline-block; position: relative; margin: 0px 5px; border: solid 1px #ccc;">
								<img class="image" src="${imageUrl}/${attachment.url}" />
								<a class="imageClose" href="javascript:deleteImagePanel2(${sum})"></a>
								<input type="hidden" path="attachments2[${sum}].id" name="attachments2[${sum}].id" id="attachments2[${sum}].id"
									value="${attachment.id}" />
								<input type="hidden" path="attachments2[${sum}].url" name="attachments2[${sum}].url"
									id="attachments1[${sum}].url" value="${attachment.url}" />
							</div>
							<c:set value="${sum + 1}" var="sum" />
						</c:forEach>
