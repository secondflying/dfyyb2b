...
<c:set var="pageTitle" value="问题" scope="request" />
...
<section class="well shadow">
	<fieldset>
		<legend>
			问题详情
			<c:if test="${size>0}">
				<button class="btn btn-info pull-right" type="button" onclick="downloadImg(${question.id})">下载图片</button>
			</c:if>
			<button class="btn btn-danger pull-right" type="button" onclick="convert(${question.id})">转成拉拉呱</button>
		</legend>
				...
				<fieldset>
					<div class="row" style="margin-left: 10px;">
						...
						<div style="display: inline-block;">
							...
							<div style="text-align: left;">
								<label style="font-size: 10px; display: inline-block;">时间：<fmt:formatDate value='${question.time}'
										type='date' pattern='yyyy-MM-dd HH:mm:ss' /></label>
							</div>
...
<script>
	...
	function convert(id) {
		bootbox.confirm("确定将问题转成拉拉呱吗？", "取消", "确定", function(isOk) {
			if (!isOk) {
				return;
			}
	...
	function downloadImg(id) {
		window.location.href = '<c:url value="downloadImg" />?id='+id;
	}
