...
<body>
	<div class="container">
		...
		<c:forEach items="${agrotechnical.attachments}" var="att">
			<div class="row">
				<img class="" src="${imageUrl}/${att.url}" />
			</div>
		</c:forEach>
		<%-- 		<p style="padding: 10px 0 0;font-size:18px;line-height: 25px;color:#505050;font-weight:normal;" class="lead">${agrotechnical.content}</p> --%>
		<c:forEach items="${contents}" var="con">
			<p style="padding: 5px 0 0 0; font-size: 18px; line-height: 25px; color: #505050; font-weight: normal;">${con}</p>
		</c:forEach>
