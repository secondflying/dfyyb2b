...
<c:set var="pageTitle" value="商品单位管理" scope="request" />
...
<section class="well">
	<fieldset>
		<legend>
			商品单位
			<c:url var="newUrl" value="newunit" />
		...
		<table id="objList" class="table  table-bordered table-hover table-striped" style="margin: 0;">
			...
			<tbody>
				...
				<c:forEach items="${units}" var="unit">
					<tr>
						<td style="width: 200px;"><img style="width: 120px; height: 90px;" src="${imageUrl}/${unit.image}"
							id="diseaseImage" /></td>
						<td style="width: 200px;"><b><c:out value="${unit.name}" /></b></td>
						<td style=""><c:url var="eUrl" value="editunit?id=${unit.id}" /> <a
								id="editone" class="btn btn-small btn-warning" href="${eUrl}">编辑 </a> 
							<button class="btn btn-small btn-danger" type="button" onclick="deleteOne(${unit.id})">删除</button>
						</td>
					</tr>
				</c:forEach>
			</tbody>
...
$(document).ready(function() {
    ...
    var options = {
            ...
            onPageClicked: function (event, originalEvent, type, page) { 
            	var page = page-1; 
        		window.location.href = '<c:url value="units" />?page=' + page + "&size=" + size;
            }
    };
