...
<c:set var="pageTitle" value="编辑商品单位" scope="request" />
...
<div class="navbar">
	<div class="navbar-inner">
		<ul class="nav">
			...
			<li class="active"><a href="${unitUrl2}">商品单位</a></li>
		</ul>
	</div>
</div>
<section class="well">
	<c:url var="perfectUrl" value="/manager/parameters/editunit" />
	<form id="crop" action="${perfectUrl}" method="post" enctype="multipart/form-data">
		<fieldset>
			...
			<div class="control-group">
				...
				<div class="controls">
					<input path="name" name='name' id="name" class="input-xxlarge" placeholder="输入名称" value = "${unit.name }" required />
			...
			<p id="attachmenttip" class="text-error">请上传单位图片</p>
			<div id="attachmentsdiv">
				<c:if test="${unit.image != null }">
					<div id="panel1"
						style="width: 150px; height: 150px; display: inline-block; position: relative; margin: 0px 5px; border: solid 1px #ccc;">
						<img class="image" src="${imageUrl}/${unit.image}" />
						<a class="imageClose" href="javascript:deleteImagePanel(1)"></a>
						<input type="hidden" path="image" name="image" id="image" value="${unit.image}" />
					</div>
				</c:if>
			</div>
		</fieldset>
...
$(document).ready(function () {
	...
	$("#multiple_file_upload").uploadify({
		...
		'uploader' : '<c:url value="unitbash;jsessionid=${pageContext.session.id}"/>',
