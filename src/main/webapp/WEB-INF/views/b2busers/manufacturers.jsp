...
<c:set var="pageTitle" value="厂家管理" scope="request" />
		...
		<table id="objList" class="table  table-bordered table-hover table-striped" style="margin: 0;">
			...
			<tbody>
				...
				<c:forEach items="${users}" var="user">
					<tr class="${cssClass}">
						<c:url var="infoUrl" value="manufactureinfo?id=${user.id}" />
...
$(document).ready(function() {
    ...
    var options = {
            ...
            onPageClicked: function (event, originalEvent, type, page) { 
        		...
        		window.location.href = '<c:url value="manufacturers" />?keyword='+ encodeURIComponent(text) + "&page=" + page + "&size=" + size;
            }
    };
...
function search(){
	var text = $("#searchtxt").val();
	window.location.href = '<c:url value="manufacturers" />?keyword='+encodeURIComponent(text);
}
</script>
