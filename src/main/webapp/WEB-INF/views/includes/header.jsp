				...
				<div class="nav-collapse">
					<ul class="nav" style="width: 940px;">
						<c:forEach items="${functions}" var="fun">
							<c:set var="cssClass" value="topbar" />
							<c:if test="${fn:contains(requestScope['javax.servlet.forward.servlet_path'],fun.url)}">
								<c:set var="cssClass" value="active topbar" />
							</c:if>
							<c:url var="url" value="${fun.url}/index" />
							<li class="${cssClass}"><a id="infoLink" href="${url}">${fun.name}</a></li>
						</c:forEach>


						<c:set var="cssClass" value="topbar" />
						<c:if test="${fn:contains(requestScope['javax.servlet.forward.servlet_path'], '/manager/apps')}">
							<c:set var="cssClass" value="active topbar" />
						</c:if>
						<c:url var="appUrl" value="/manager/apps/download" />
						<li class="${cssClass}"><a id="appLink" href="${appUrl}">应用下载</a></li>
					</ul>
				</div>
