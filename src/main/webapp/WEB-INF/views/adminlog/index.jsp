...
<c:set var="pageTitle" value="管理员操作日志" scope="request" />
	...
	<fieldset>
		<legend>
			...
			<div class="input-append pull-right">
				<input id="searchtxt" cssClass="input-xlarge" style="line-height: 24px;" placeholder="输入用户名" />
		...
		<table id="objList" class="table  table-bordered table-hover table-striped" style="margin: 0;">
			...
			<tbody>
				...
				<c:forEach items="${logs}" var="log">
					<tr class="${cssClass}">
						<td style="width:120px;"><c:out value="${log.user.name}" /></td>
						<td><c:out value="${log.description}" /></td>
						<td style="width:220px;"><c:out value="${log.time}" /></td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
